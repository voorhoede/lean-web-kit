<template>
  <nav class="table-of-contents" :class="{ 'table-of-contents--open' : tableOfContentsIsOpen }">
    <h2 class="a11y-sr-only">{{ 'table_of_contents' | t }}</h2>
    <button class="table-of-contents__button" @click="toggleTableOfContents" >{{ 'table_of_contents' | t }}
      <span v-if="tableOfContentsIsOpen" class="a11y-sr-only">{{ 'close_menu' | t }}</span>
      <span v-else class="a11y-sr-only">{{ 'open_menu' | t }}</span>
    </button>
